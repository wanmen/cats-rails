class List < ActiveRecord::Base
  ...
  scope :best3Booklist,->{select("lists.id, lists.title, count(rates.id) AS rates_count").where(list_type: 1).joins(:rates).group("lists.id").order("rates_count DESC").limit(3)}
  scope :best3Videolist,->{select("lists.id, lists.title, count(rates.id) AS rates_count").where(list_type: 2).joins(:rates).group("lists.id").order("rates_count DESC").limit(3)}
end
