class List < ActiveRecord::Base
  ...
  scope :best6Booklist,->{select("lists.id, lists.title, lists.rate").where(list_type: 1).group("lists.id").order("lists.rate DESC").limit(6)}
  scope :best6Videolist,->{select("lists.id, lists.title, lists.rate").where(list_type: 2).group("lists.id").order("lists.rate DESC").limit(6)}
end
